...
const WALL_ALIGN_OPTIONS = [
  ...
  { label: "Outer", value: "outer" as const },
  { label: "Inner", value: "inner" as const },
];
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-1">
                {WALL_ALIGN_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => setWallAlignment(opt.value)}
                    className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${
            ...
            <div className="flex items-center justify-between">
              <label className="text-sm text-muted">Offset Dist (m)</label>
              <input
                type="number"
                min={0.05}
                max={2}
                ...
                onChange={(e) =>
                  setOffsetDistance(Math.max(0.05, parseFloat(e.target.value) || 0.05))
                }
                className="w-20 text-right text-sm font-mono tabular-nums bg-surface-alt border border-border rounded px-2 py-0.5 focus:outline-none focus:border-accent"
              />
            </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {COLUMN_ALIGN_OPTIONS.map((opt) => (
              <button
                key={opt.value}
                onClick={() => setColumnAlignMode(opt.value)}
                className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${
                  columnAlignMode === opt.value
                    ? "bg-accent text-white border-accent"
                    : "bg-surface-alt text-muted border-border hover:border-accent"
                }`}
              >
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-mono tabular-nums">{openings.length}</span>
            </div>
            <div className="flex items-center justify-between">
              <label className="text-sm text-muted">Columns</label>
              <span className="text-sm font-mono tabular-nums">{columns.length}</span>
            </div>
            <div className="flex items-center justify-between">
              <label className="text-sm text-muted">Furniture</label>
              <span className="text-sm font-mono tabular-nums">{furniture.length}</span>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div>
              ...
              <div className="space-y-3">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const { x2, y2 } = wallLengthFromMeters(
                        ...
                        selectedWall.x2, selectedWall.y2, newLen, 50
                      );
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    <button
                      onClick={() => updateWall(selectedWall.id, { wallType: "interior", thickness: getDefaultThickness("interior") })}
                      className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${
                        selectedWall.wallType === "interior" ? "bg-accent text-white border-accent" : "bg-surface-alt text-muted border-border hover:border-accent"
                      }`}>Int</button>
                    <button
                      onClick={() => updateWall(selectedWall.id, { wallType: "exterior", thickness: getDefaultThickness("exterior") })}
                      className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${
                        selectedWall.wallType === "exterior" ? "bg-accent text-white border-accent" : "bg-surface-alt text-muted border-border hover:border-accent"
                      }`}>Ext</button>
                  </div>
                </div>
                <div className="flex items-center justify-between">
        ...
        {selectedOpening && (
          <>
            ...
            <div>
              <h3 className="text-xs font-medium text-muted mb-2">
                {selectedOpening.type === "door" ? "Door" : selectedOpening.type === "sliding_door" ? "Sliding Door" : "Window"}
              </h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-sm text-muted">Type</label>
                  <span className="text-sm font-mono tabular-nums capitalize">{selectedOpening.type.replace("_", " ")}</span>
                </div>
                <div className="flex items-center justify-between">
                  <label className="text-sm text-muted">Width (m)</label>
                ...
                <button onClick={() => deleteOpening(selectedOpening.id)}
                  ...
                  Delete {selectedOpening.type === "door" ? "Door" : selectedOpening.type === "sliding_door" ? "Sliding Door" : "Window"}
                </button>
              </div>
            </div>
          </>
        ...
        {selectedColumn && (
          <>
            <div className="w-full h-px bg-border" />
            <div>
              <h3 className="text-xs font-medium text-muted mb-2">Structural Column</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-sm text-muted">Width (m)</label>
        ...
        {selectedLabel && (
          <>
            <div className="w-full h-px bg-border" />
            <div>
              <h3 className="text-xs font-medium text-muted mb-2">Room Label</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                ...
                <div className="flex flex-wrap gap-1">
                  {FILL_TYPE_OPTIONS.map((opt) => (
                    <button key={opt.value}
                      onClick={() => updateFloorFill(selectedFill.id, { fillType: opt.value })}
                      className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${
          ...
          <>
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <label className="text-sm text-muted">Rotation</label>
                  <span className="text-sm font-mono tabular-nums">{selectedFurniture.rotation}&deg;</span>
                </div>
                <div>
                  ...
                  <div className="flex justify-between text-[9px] text-muted/50 font-mono mt-0.5">
                    <span>0&deg;</span><span>180&deg;</span><span>360&deg;</span>
                  </div>
                </div>
                <button onClick={() => updateFurniture(selectedFurniture.id, { rotation: (selectedFurniture.rotation + 90) % 360 })}
                  className="w-full text-xs text-accent border border-accent/30 rounded px-2 py-1 hover:bg-accent/10 mt-1">Rotate 90&deg;</button>
        ...
        {!selectedWall && !selectedOpening && !selectedColumn && !selectedLabel && !selectedFill && !selectedFurniture && (
